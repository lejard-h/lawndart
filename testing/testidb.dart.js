...
_IDBRequestEventsImpl.prototype.get$success = function() {
...
_IDBTransactionEventsImpl.prototype.get$abort = function() {
  return this._get("abort");
}
_IDBTransactionEventsImpl.prototype.get$complete = function() {
  return this._get("complete");
}
...
function IndexedDb(dbName, storeNames, version) {
  this.isReady = false;
  this.dbName = dbName;
...
IndexedDb.prototype.open = function() {
  ...
  print$(("requested open for " + this.dbName));
  request.get$on().get$success().add($wrap_call$1((function (e) {
    print$("success");
    $this._db = e.get$target().get$result();
    $this._initDb(completer);
  })
  ), false);
  request.get$on().get$error().add$1((function (e) {
    print$("error");
...
IndexedDb.prototype._initDb = function(completer) {
  var $this = this;
  if (this.version != this._db.version) {
    print$(("upgrading " + this._db.version + " to " + this.version + " for " + this.dbName));
    var versionChange = this._db.setVersion(this.version);
    versionChange.get$on().get$success().add($wrap_call$1((function (e) {
      $this.storeNames.forEach$1((function (storeName) {
        if ($eq$($this._db.objectStoreNames.indexOf$1(storeName), (-1))) {
    ...
    versionChange.get$on().get$error().add($wrap_call$1((function (e) {
      completer.completeException(e);
    })
    ), false);
  }
  else {
    print$(("version good to go for " + this.dbName));
    this.isReady = true;
    completer.complete(true);
  }
}
IndexedDb.prototype.store = function(storeName) {
  if (!this.isReady) {
    $throw("Database not opened or ready");
  }
  if ($eq$(this.storeNames.indexOf$1(storeName), (-1))) {
    $throw(("Store name " + storeName + " is unknown to database " + this.dbName));
  }
  return new _IndexedDbAdapter(this._db, storeName);
}
function _IndexedDbAdapter(_db, storeName) {
  this._db = _db;
  this.storeName = storeName;
}
_IndexedDbAdapter.prototype._createTxn = function(type, completer, onComplete) {
  var mode = (type == "readwrite") ? (1) : (0);
  print$(("opening " + type + " txn for store " + this.storeName));
  var txn = this._db.transaction(this.storeName, mode);
  txn.get$on().get$complete().add($wrap_call$1((function (e) {
    print$("txn completed");
    completer.complete(onComplete(e));
  })
  ), false);
  txn.get$on().get$error().add($wrap_call$1((function (e) {
    print$(("txn error: " + e));
    completer.completeException(e.get$target().get$error());
  })
  ), false);
  txn.get$on().get$abort().add($wrap_call$1((function (e) {
    print$(("txn aborted: " + e));
...
_IndexedDbAdapter.prototype.save = function(obj, key) {
  var completer = new CompleterImpl();
  var jsonObj = json_JSON.stringify(obj);
  var txn = this._createTxn("readwrite", completer, (function (e) {
    return key;
  })
  );
...
function main() {
  var idb = new IndexedDb("chat-db", ["chat-db", "delayed-messages"], "4");
  var delayedMessagesDb;
  var test = idb.open();
  test.handleException((function (e) {
    return print$(("ERROR: " + e));
  })
  );
  test.chain((function (_) {
